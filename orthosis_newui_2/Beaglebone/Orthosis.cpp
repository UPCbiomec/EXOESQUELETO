    ...
    status(0)
{
    ...
    connect(this, &Orthosis::motorflexion_right, Mtr1.get(), &maxonMotor::flexion_right);
    connect(this, &Orthosis::motorflexion_left, Mtr2.get(), &maxonMotor::flexion_left);
    connect(this, &Orthosis::motorextension_right, Mtr1.get(), &maxonMotor::extension_right);
    connect(this, &Orthosis::motorextension_left, Mtr2.get(), &maxonMotor::extension_left);
    connect(this, &Orthosis::motor_sitdown, Mtr1.get(), &maxonMotor::sitdown);
    connect(this, &Orthosis::motor_sitdown, Mtr2.get(), &maxonMotor::sitdown);
    connect(this, &Orthosis::motor_standup, Mtr1.get(), &maxonMotor::standup);
    connect(this, &Orthosis::motor_standup, Mtr2.get(), &maxonMotor::standup);
...
// HOMING
void Orthosis::EXTENSION_RIGHT()
{

    emit motorextension_right();
}


// 90 degrees
void Orthosis::FLEXION_RIGHT()
{
    // Send motors to corresponding positions
    emit motorflexion_right();
}

// HOMING
void Orthosis::EXTENSION_LEFT()
{

    emit motorextension_left();
}


// 90 degrees
void Orthosis::FLEXION_LEFT()
{
    // Send motors to corresponding positions
    emit motorflexion_left();
}

// SITDOWN
void Orthosis::SITDOWN()
{
...
void Orthosis::readPendingDatagrams()
{
    while (socket.hasPendingDatagrams())
    {
        ...
        else if (message == QString("EXTENSIONRIGHT"))
        {
             std::cout << "EXTENSION"<< std::endl;
             EXTENSION_RIGHT();
             socket.writeDatagram(QByteArray("Ok"), UDPClient, cmdPort);
        }
        else if (message == QString("FLEXIONRIGHT"))
        {
             std::cout << "FLEXION"<< std::endl;
             FLEXION_RIGHT();
             socket.writeDatagram(QByteArray("Ok"), UDPClient, cmdPort);
        }
        else if (message == QString("EXTENSIONLEFT"))
        {
             std::cout << "EXTENSION"<< std::endl;
             EXTENSION_LEFT();
             socket.writeDatagram(QByteArray("Ok"), UDPClient, cmdPort);
        }
        else if (message == QString("FLEXIONLEFT"))
        {
             std::cout << "FLEXION"<< std::endl;
             FLEXION_LEFT();
             socket.writeDatagram(QByteArray("Ok"), UDPClient, cmdPort);
        }
        else if (message == QString("SITDOWN"))
        {
             std::cout << "SIT DOWN"<< std::endl;
             SITDOWN();
             socket.writeDatagram(QByteArray("Ok"), UDPClient, cmdPort);
        }
        else if (message == QString("STANDUP"))
        {
