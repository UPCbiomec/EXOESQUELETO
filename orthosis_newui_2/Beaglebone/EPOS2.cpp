...
maxonMotor::maxonMotor(const bool rev, const long offset) : reverse(rev), hoffset(offset), data(2)
{
    ...
    // Reset motor state
    errChk(VCS_ResetDevice(keyHandle, motor, &errid));
    errChk(VCS_ClearFault(keyHandle, motor, &errid));
    errChk(VCS_SetDisableState(keyHandle, motor, &errid));

    //Configure switch as Quick Stop
    errChk(VCS_DigitalInputConfiguration(keyHandle, motor, 1, 5, true, false, true, &errid));
...
void maxonMotor::extension_right()
{
    errChk(VCS_SetOperationMode(keyHandle, motor, OMD_PROFILE_POSITION_MODE, &errid));
    errChk(VCS_ActivateProfilePositionMode(keyHandle, motor, &errid));
    errChk(VCS_SetPositionProfile(keyHandle, motor, 500, 10000, 10000,&errid));
    errChk(VCS_MoveToPosition(keyHandle, motor, -150000, 0, 0, &errid));
...
//EXTENSION OF THE RIGHT LEG
void maxonMotor::extension_left()
{
    errChk(VCS_SetOperationMode(keyHandle, motor, OMD_PROFILE_POSITION_MODE, &errid));
    errChk(VCS_ActivateProfilePositionMode(keyHandle, motor, &errid));
    errChk(VCS_SetPositionProfile(keyHandle, motor, 500, 10000, 10000,&errid));
...
//
//FLEXION OF THE RIGHT LEG
void maxonMotor::flexion_right()
{
    errChk(VCS_SetOperationMode(keyHandle, motor, OMD_PROFILE_POSITION_MODE, &errid));
    errChk(VCS_ActivateProfilePositionMode(keyHandle, motor, &errid));
    errChk(VCS_SetPositionProfile(keyHandle, motor, 500, 10000, 10000,&errid));
...
//FLEXION OF THE LEFT LEG
void maxonMotor::flexion_left()
{
    errChk(VCS_SetOperationMode(keyHandle, motor, OMD_PROFILE_POSITION_MODE, &errid));
    errChk(VCS_ActivateProfilePositionMode(keyHandle, motor, &errid));
    errChk(VCS_SetPositionProfile(keyHandle, motor, 500, 10000, 10000,&errid));
...
{
    errChk(VCS_GetAllDigitalInputs(keyHandle, motor, &pInput, &errid));
    if (m == motor)
    {       
...
// Motor go-to-position command (only if switch is not pressed)
