...
void maxonMotor::read(double t)
{
    ...
    //errChk(VCS_GetCurrentIs(keyHandle, motor, &pCurrentIs, &errid));
    errChk(VCS_GetCurrentIsAveraged(keyHandle, motor, &pCurrentIsAveraged, &errid));

    double pos = static_cast<double>(qcs) / QC_PER_DEG;
    double current = static_cast<double>(pCurrentIs);
    double current_averaged = static_cast<double>(pCurrentIsAveraged);

    data[0].push_back(t);
    //data[1].push_back(pos);
    //data[2].push_back(current);
    data[1].push_back(current_averaged);
